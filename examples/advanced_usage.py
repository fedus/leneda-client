...
def parse_arguments():
    ...
    # API credentials
    parser.add_argument("--api-key", help="Your Leneda API key (or set LENEDA_API_KEY environment variable)")
    parser.add_argument("--energy-id", help="Your Energy ID (or set LENEDA_ENERGY_ID environment variable)")

    # Other parameters
    parser.add_argument("--metering-point", default="LU-METERING_POINT1", 
                        help="Metering point code (default: LU-METERING_POINT1)")
    parser.add_argument("--example", type=int, choices=[1, 2, 3, 4], default=0,
...
def get_credentials(args):
    ...
    if not api_key:
        logger.error("API key not provided. Use --api-key or set LENEDA_API_KEY environment variable.")
        sys.exit(1)

    if not energy_id:
        logger.error("Energy ID not provided. Use --energy-id or set LENEDA_ENERGY_ID environment variable.")
        sys.exit(1)
...
def convert_aggregated_to_dataframe(aggregated_data: AggregatedMeteringData) -> pd.DataFrame:
...
def compare_consumption_periods(
    ...
    plt.subplot(2, 1, 2)
    plt.bar(range(len(daily2)), daily2['value'], label=f'Period 2 ({period2_str})', color='orange')
...
def analyze_monthly_trends(
    ...
    print(f"  - Highest consumption month: {max_month} ({df['value'].max():.2f} {df['unit'].iloc[0]})")
    print(f"  - Lowest consumption month: {min_month} ({df['value'].min():.2f} {df['unit'].iloc[0]})")
...
def detect_consumption_anomalies(
    ...
    print(f"  - Threshold for anomaly: {threshold:.2f} {df['unit'].iloc[0]} ({threshold_pct}% of mean)")
    ...
    if not anomalies.empty:
        ...
        for idx, row in anomalies.head(5).iterrows():
            ...
            print(f"  - {idx.strftime('%Y-%m-%d %H:%M')}: {row['value']:.2f} {row['unit']} "
                  f"(Deviation: {deviation_pct:.2f}%)")
        
        # Plot the data with anomalies highlighted
        plt.figure(figsize=(12, 6))
        plt.plot(df.index, df['value'], marker='.', linestyle='-', markersize=2, label='Normal')
        ...
        plt.axhline(y=mean + threshold, color='orange', linestyle='--', 
    ...
    try:
        ...
        if example_num == 0 or example_num == 1:
            ...
            plot_consumption_data(
                ...
                save_path=os.path.join(output_dir, "hourly_consumption.png") if save_plots else None
            )
        ...
        if example_num == 0 or example_num == 2:
            ...
            compare_consumption_periods(
                ...
                current_week_start,
                current_week_end,
                save_path=os.path.join(output_dir, "weekly_comparison.png") if save_plots else None
            )
        ...
        if example_num == 0 or example_num == 3:
            ...
            analyze_monthly_trends(
                ...
                metering_point, 
                year,
                save_path=os.path.join(output_dir, f"monthly_trends_{year}.png") if save_plots else None
            )
