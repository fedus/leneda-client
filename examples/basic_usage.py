...
from leneda import ElectricityConsumption
    ...
    try:
        ...
        if consumption_data.items:
            ...
            for item in consumption_data.items[:3]:
                print(f"Time: {item.started_at.isoformat()}, Value: {item.value} {consumption_data.unit}, "
        ...
        if monthly_consumption.aggregated_time_series:
            ...
            for item in monthly_consumption.aggregated_time_series:
                print(f"Period: {item.started_at.strftime('%Y-%m')} to {item.ended_at.strftime('%Y-%m')}, "
